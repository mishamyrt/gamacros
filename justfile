...
mem-scenario duration='10': _sdl2-lib-env
  ...
  echo "baseline_rss_kb=$(ps -o rss= -p $PID | tr -d ' ')" | tee /tmp/gamacrosd_mem_metrics.txt
  echo "Perform a couple of joystick events now (press 2 buttons, move a stick)" | tee -a /tmp/gamacrosd_mem_metrics.txt
  # Allow time for interaction
  sleep {{duration}}
  # After-interaction snapshot
  echo "after_rss_kb=$(ps -o rss= -p $PID | tr -d ' ')" | tee -a /tmp/gamacrosd_mem_metrics.txt
  # Optional deep summary if tools exist
  if command -v vmmap >/dev/null; then
    /usr/bin/vmmap -summary $PID | egrep 'Physical footprint|PhysFootprint' | tee -a /tmp/gamacrosd_mem_metrics.txt || true
  fi
  ...
  awk 'BEGIN{print "metric,value"} /rss_kb/{split($0,a,"="); print a[1]","a[2]}' /tmp/gamacrosd_mem_metrics.txt > /tmp/gamacrosd_mem.csv
...
mem-xctrace duration='15':
  ...
  xcrun xctrace record --template 'Memory Usage' --time-limit {{duration}}s --output "$OUT" --launch "$BIN"
