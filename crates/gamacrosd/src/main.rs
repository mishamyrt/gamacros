...
use gamacros_profile::{Profile, RuntimeAction, TriggerPhase};
...
fn main() {
    ...
    // Activity monitor must run on the main thread. We'll bridge its std::mpsc receiver to crossbeam.
    ...
    let event_loop = std::thread::spawn(move || {
        ...
        // Load profile once (v1). Later: add file watch and hot-reload.
        ...
        let mut runtimes: HashMap<u32, gamacros_profile::ProfileRuntime> = HashMap::new();
        ...
        for info in manager.controllers() {
            let runtime = profile.build_runtime(info.vendor_id, info.product_id, &current_app);
...
fn dispatch_action(keypress: &mut Performer, manager: &ControllerManager, id: u32, action: RuntimeAction) {
